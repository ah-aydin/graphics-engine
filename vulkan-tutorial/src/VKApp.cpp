...
void VKApp::initWindow()
{
	...
	{
		throw std::runtime_error("Failed to initilialize GLFW!");
	}
...
void VKApp::drawFrame()
{
	...
	// Reset if the checks passed to avoid deadlock
...
void VKApp::createSyncObjects()
{
	...
	for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
	{
		...
		{
			throw std::runtime_error("Faield to create sync objects");
		}
	}
}
...
bool VKApp::checkValidationSupport()
{
	uint32_t layerCount;
...
VKAPI_ATTR VkBool32 VKAPI_CALL VKApp::debugCallback(
	...
	{
		std::cerr << "HORRIBLE ERROR OCCURED\n";
	}
...
void VKApp::createSurface()
{
	...
	{
		throw std::runtime_error("Failed to create surface for the window");
	}
}
...
int VKApp::rateDeviceSuitability(VkPhysicalDevice device)
{
	...
	// Check maximum size of textures
...
QueueFamilyIndicies VKApp::findQueueFamilies(VkPhysicalDevice device)
{
...
VkExtent2D VKApp::chooseSwapExtend(const VkSurfaceCapabilitiesKHR& capabilities)
{
...
void VKApp::createSwapChain()
{
	...
	createInfo.preTransform = swapChainSupport.capabilities.currentTransform; // No transformations, so keep the current one
	createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR; // Ignore blending with other windows
...
void VKApp::createImageViews()
{
	...
	for (size_t i = 0; i < swapChainImages.size(); ++i)
	{
		...
		// Do not swizzle the color channels
...
void VKApp::createGraphicsPipeline()
{
	...
	// Remove these from the defaults
	// Will have to specify them during drawing time
	...
	viewportState.pViewports = &viewport; // If not dynamic
	viewportState.scissorCount = 1;
	viewportState.pScissors = &scissor; // If not dynamic

	VkPipelineRasterizationStateCreateInfo rasterizer{};
	rasterizer.sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;
	rasterizer.depthClampEnable = VK_FALSE; // If VK_TRUE, (GPU FEATURE [depthClamp])
	rasterizer.rasterizerDiscardEnable = VK_FALSE; // If VK_TRUE, geometry never passes to the rasterizer stage (GPU feature [rasterizerDiscard])
	rasterizer.polygonMode = VK_POLYGON_MODE_FILL; // If not FILL (GPU feature [polygonMode])
	rasterizer.lineWidth = 1.0f; // Greater than 1.0f, required (GPU FEATURE [wideLines])
	...
	multisampling.sampleShadingEnable = VK_FALSE; // If VK_TRUE (GPU feature)
	...
	colorBlendAttachment.blendEnable = VK_TRUE; // The rest of the parameters are important if VK_TRUE
	...
	colorBlending.logicOpEnable = VK_FALSE; // Settings this to VK_TRUE, disable blendEnable from colorBlendAttachment
...
void VKApp::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
	...
	cpyRegion.size = size;
	vkCmdCopyBuffer(commandBuffer, srcBuffer, dstBuffer, 1, &cpyRegion);
