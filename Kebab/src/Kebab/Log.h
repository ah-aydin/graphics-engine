...
namespace kbb
{
	...
	#define KBB_CORE_ASSERTION(check, ...)

	#define KBB_TRACE(...)
	#define KBB_INFO(...)
	#define KBB_WARN(...)
	#define KBB_ERROR(...)
	#define KBB_ASSERTION(check, ...)
	...
	#define KBB_CORE_ERROR(...)				kbb::Log::GetCoreLogger().error("File {0} - Line {1}", __FILE__, __LINE__);kbb::Log::GetCoreLogger().error(__VA_ARGS__);
	#define KBB_CORE_ASSERTION(check, ...)	if (!check) { kbb::Log::GetCoreLogger().error(__VA_ARGS__); assert(false); }
	...
	#define KBB_ERROR(...)					kbb::Log::GetClientLogger().error("File {0} - Line {1}", __FILE__, __LINE__);kbb::Log::GetClientLogger().error(__VA_ARGS__);
	#define KBB_ASSERTION(check, ...)		if (!check) { kbb::Log::GetClientLogger().error(__VA_ARGS__); assert(false); }
