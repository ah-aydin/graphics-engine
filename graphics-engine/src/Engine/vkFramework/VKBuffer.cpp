...
)
{
	...
	VK_CALL_RET_BOOL(vkCreateImage(device, &imageInfo, nullptr, &image));
	...
	allocInfo.allocationSize = memReqs.size;
	allocInfo.memoryTypeIndex = findMemoryType(physicalDevice, memReqs.memoryTypeBits, properties);
	VK_CALL_RET_BOOL(vkAllocateMemory(device, &allocInfo, nullptr, &imageMemory));
...
bool createTextureSampler(VkDevice device, VkSampler* sampler)
{
	...
	VK_CALL_RET_BOOL(vkCreateSampler(device, &samplerInfo, nullptr, sampler));
...
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommand(vulkanDevice);
	...
	endSingleTimeCommand(vulkanDevice, commandBuffer);
}
